...
const getExtractFunctions = (extractFunctions, body, postProcess = null) => {
  for (const extractFunction of extractFunctions) {
    try {
      ...
      // Procesamos el código con Babel Standalone
      const transformedCode = Babel.transform(postProcess ? postProcess(func) : func, {
        presets: ['env'], // Asegura compatibilidad general con ECMAScript
      }).code;

      // Convertimos el código transformado a una función ejecutable
      return new Function('context', `${transformedCode} return context;`);
    } catch (err) {
      continue;
    }
  }
